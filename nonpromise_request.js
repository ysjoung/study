...
//eventEmitter.on('request', requestHandler );
//eventEmitter.on('select', selectHandler);
...
function allselectHandler(o, p) {
  ...
  if (options.allselectBool)
    ...
    var allselectquery = `select * from (` + query + options.where + `) A where A.rnum between ${o} AND ${p}`;
*/
  ...
  options.gconnection.execute(allselectquery, function (err, result) {
    ...
    for (var i = 0; i < result.rows.length; i++) {
      ...
      /* 값 대입하기
      myrequest.body.key = r[1];
      myrequest.body.calling = r[1];
      myrequest.body.startTime = mystarttime;
      myrequest.body.called = r[2];
      myrequest.body.endTime = myendtime;
      myrequest.body.duration = r[4];
      */

      options.countr0.push(r[0]);
      //console.log(options.countr0);
